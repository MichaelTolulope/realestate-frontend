...
const AddProperty = ({ show }) => {
    ...
    const handlePropertyImageInput = (e) => {
        ...
        setPropertyImages(targetFiles)

        targetFiles.map(image => (
            imagePreviewList.push(URL.createObjectURL(image))
        ))

        setImagesPreview(imagePreviewList)
    ...
    const handleAddAmmenity = () => {
        setAmmenities((prevAmmenities) => {
            const updatedAmmenities = [...prevAmmenities, ammenity];
            console.log(updatedAmmenities); // Correctly logs the updated ammenities array
            return updatedAmmenities;
        });
    ...
    const handleAddTag = () => {
        setTags((prevTags) => {
            ...
            console.log(updatedTags); // Correctly logs the updated Tags array
            return updatedTags;
        });
        console.log(tags);
        setTag('')
    ...
    const handleSubmit = async (e) => {
        ...
        // Use toast.promise to handle the feedback UI
    ...
    const propertyPromise = async () => {
        ...
        const response = await createNewProperty(formData);

        if (!response.ok) {
            const error = await response.json();
            throw new Error(error.message); // Throw error to trigger toast error
        }


        return { message: "Property upload successful, awaiting approval" }; // Return success data
    };
