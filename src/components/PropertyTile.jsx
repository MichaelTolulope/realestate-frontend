...
const PropertyCard = ({ property }) => {
    const { images, title, location, price, _id } = property;
    const { bedrooms, bathrooms, area } = property.features;
        ...
        <div className=" max-w-[350px] rounded-2xl cursor-pointer" onClick={handleClick}>
            ...
            <div className="relative w-full h-[230px] overflow-hidden rounded-2xl">
                {/* <img
                    ...
                    alt={title}
                    className="w-full h-full object-cover rounded-2xl hover:scale-105 transition-transform"
                /> */}
                    ...
                    {images.map((img, index) => (
                        <SwiperSlide key={index}>
                            <div className="w-full relative">
                                <motion.img key={img.url} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                                 src={img.url} width={350} height={230} alt="property img" className="object-cover w-full h-[230px] hover:scale-105 transition-transform" />
                                <button onClick={(e)=> e.stopPropagation()} className="z-50 absolute top-3 right-3 bg-white text-gray-800 rounded-full p-2 shadow hover:bg-gray-100">
                                    ❤️
                                </button>
                            </div>
                        </SwiperSlide>
